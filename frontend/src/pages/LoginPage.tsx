...
import { authLogin, authMFAVerify, authOAuthGetUrl, authGetMe, authGetOAuthProviders } from '../services/api';
...
export const LoginPage: React.FC = () => {
    ...
    const tempTokenRef = useRef(''); // stores temp_token — ref immune to stale closures
    ...
    // Query which OAuth providers are configured on this server
    useEffect(() => {
        authGetOAuthProviders().then(setOAuthProviders).catch(() => { /* keep defaults (both false) */ });
    }, []);

    // Detect OAuth → MFA redirect: the OAuthCallback stores a temp_token in sessionStorage
    // and navigates here with ?mfa_pending=1 when the user has MFA enabled.
    // Also surface friendly error messages from SSO failures (?sso_error=...).
    useEffect(() => {
        const params = new URLSearchParams(window.location.search);
        const pending = params.get('mfa_pending');
        const ssoError = params.get('sso_error');
        if (pending === '1') {
            const t = sessionStorage.getItem('mfa_temp_token');
            if (t) {
        ...
        if (ssoError) {
            const messages: Record<string, string> = {
                ...
                no_email: 'No email address was returned by the identity provider.',
    ...
    const handleCredentials = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if ('mfa_required' in res && res.mfa_required) {
                tempTokenRef.current = (res as any).temp_token as string;
                setPhase('mfa');
            } else {
                // Full token — fetch profile with the new token (store not yet updated)
                const token = (res as any).access_token as string;
                const profile = await authGetMe(token);
                login(token, profile);
                navigate('/');
            }
        } catch (err: any) {
    ...
    const handleMFA = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            const res = await authMFAVerify(tempTokenRef.current, mfaCode, useRecovery);
            // Pass the fresh token explicitly — store not yet updated at this point
            const profile = await authGetMe(res.access_token);
